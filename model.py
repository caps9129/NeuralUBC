...
class UtilityNet(nn.Module):
    ...
    def forward(self, x_emb: torch.Tensor, x_feat: torch.Tensor, domain_id: torch.Tensor, a: torch.Tensor) -> torch.Tensor:
        """
        x_emb: (B, D_emb)
        x_feat: (B, D_feat)
        domain_id: (B,)
...
def build_model_from_cfg(cfg) -> UtilityNet:
    ...
    Expects:
      ...
      cfg.pool.num_actions()
      cfg.net.* for dims/hiddens
    """
    # fallbacks, in case你還沒把這些欄位加進 config
    ...
    model = UtilityNet(
        ...
        action_embed_dim=int(cfg.net.action_embed_dim),
        domain_embed_dim=domain_embed_dim,
        emb_hidden=emb_hidden,
        feat_hidden=feat_hidden,
        hidden_sizes=tuple(cfg.net.hidden_sizes),
        dropout=float(cfg.net.dropout),
        use_layernorm=bool(cfg.net.use_layernorm),
        out_activation=str(cfg.net.out_activation),
    )
    return model