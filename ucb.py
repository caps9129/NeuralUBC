...
class PerActionLastLayerUCB:
    """
    Last-layer UCB with SHARED (joint) covariance.

    UCB(x,a) = mu(x,a) + beta * sqrt( h(x,a)^T A^{-1} h(x,a) )
    ...
    def __init__(
        ...
        self.A_inv = (1.0 / lam) * torch.eye(self.ucb_feat_dim, device=self.device, dtype=self.dtype)
    ...
    @torch.no_grad()
    def bonus_from_h(self, h: torch.Tensor) -> torch.Tensor:
        if h.dim() != 2:
        ...
        g = torch.cat([h, ones], dim=1)  # (B, H+1)

        Ainv = self.A_inv
        if Ainv.shape != (g.shape[1], g.shape[1]):
            raise ValueError(f"A_inv shape mismatch: got {tuple(Ainv.shape)}, expected {(g.shape[1], g.shape[1])}")

        tmp = torch.matmul(g, Ainv)      # (B, H+1)
    ...
    def select_action(self, model, x_emb: torch.Tensor, x_feat: torch.Tensor, domain_id: torch.Tensor):
        ...
        # keep original device to return
    ...
    def update(self, a_int: int, h_vec: torch.Tensor):
        ...
        self.A_inv = Ainv - (Ag @ Ag.t()) / float(denom)
    ...
    def reset_A(self):
        ...
        self.A_inv = (1.0 / lam) * torch.eye(P, device=self.device, dtype=self.dtype)
        self.counts = torch.zeros(self.num_actions, device=self.device, dtype=torch.long)
        self.total_updates = torch.zeros((), device=self.device, dtype=torch.long)
